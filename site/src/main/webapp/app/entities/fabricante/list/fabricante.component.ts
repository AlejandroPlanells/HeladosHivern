...
import { EntityArrayResponseType, FabricanteService } from '../service/fabricante.service';
...
export class FabricanteComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFabricante): number => this.fabricanteService.getFabricanteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fabricantes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFabricante[] | null): IFabricante[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.fabricanteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
